...
/*:
 * @plugindesc (v3.0 *) Adiciona novos efeitos de climas.
 * @author Moghunter
 *
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_Weather_EX');
    Moghunter.weatherEX_max = Number(Moghunter.parameters['Weather Max'] || 10);
...
Game_System.prototype.commandSetupWeather = function(args) {
	...
	var mode_Real = Math.min(Math.max(mode, 0),40);
	var power_Real = Math.min(Math.max(power, 1),2000);
...
SpriteWeatherEX.prototype.createSprites = function() {
    ...
    for (var i = 0; i < this.data().power; i++) {
		...
		this._sprites[i].z = i;
		this.setupBase(this._sprites[i]);
		this.refreshWeather(this._sprites[i],true);
		if ($gameSystem._weatherSprites && this.preData()) {this.loadPreData(this._sprites[i],this.preData()[i])}
...
// * Setup Base
//==============================
SpriteWeatherEX.prototype.setupBase = function(sprite) {
	sprite.anchor.x = 0.5;
	sprite.anchor.y = 0.5;
...
SpriteWeatherEX.prototype.randomPosX = function(sprite) {
    sprite._realX = -(this._cam[0] + 50 + this._modePosX) + (Math.randomInt(this.screenX() + this._screenRX + 50));
};
...
SpriteWeatherEX.prototype.randomPosY = function(sprite) {
    sprite._realY = -(this._cam[1] + 50 + this._modePosX) + (Math.randomInt(this.screenY() + this._screenRY + 70));
};
...
SpriteWeatherEX.prototype.randomPosY2 = function(sprite) {
    sprite._realY = -this._cam[1]  + Math.randomInt(this.screenY() + this._screenRY);
};
...
SpriteWeatherEX.prototype.upperPos = function(sprite,range) {
	var rg = range ? range : this._cam[0];
...
SpriteWeatherEX.prototype.bottomPos = function(sprite,range) {
 	 var rg = range ? range : this._cam[1];
	 this.randomPosX(sprite);
...
SpriteWeatherEX.prototype.refreshWeather = function(sprite,initial) {
	...
	if (initial) {
		sprite._ani[6] = 5;
		if (this.mode() != 21) {this.setPosRandom(sprite)};
	} else {
